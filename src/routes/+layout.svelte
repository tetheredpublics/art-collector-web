...
<header class="flex justify-between w-full bg-background fixed md:relative">
    ...
    <nav class={`p-4 flex ${menuIsOpen ? "translate-x-0 z-9999 top-[60px] fixed w-screen h-screen flex-col text-center space-y-4 bg-white" : "hidden md:block"}`}>  
        <a on:click={closeMenu} href="/" class="text-black/[.64] hover:text-black px-3 py-2 text-sm font-bold">Home</a>
        <a on:click={closeMenu} href="/download" class="text-black/[.64] hover:text-black px-3 py-2 text-sm font-bold">Download</a>
