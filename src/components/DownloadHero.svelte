...
<div class="flex xs:container justify-center sm:mx-auto p-4">
	{#if showControls}
		<button on:click={prevHero} class="flex-none"
			><img class="size-8" alt="navigate back" src="/icons/chevron-left.svg" /></button
		>
	{/if}
	...
	{#if showControls}
		<button on:click={nextHero} class="flex-none"
			><img class="size-8" alt="navigate next" src="./icons/chevron-right.svg" /></button
		>
	{/if}
</div>
