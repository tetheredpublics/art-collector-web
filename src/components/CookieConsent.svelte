...
{#if !$hasConsentResponse}
	<div
		class="fixed bottom-0 right-0 m-4 bg-appBackground rounded-md py-2 px-2 text-left shadow-md z-50 border-2 border-[#ADADA9]"
	>
		<div class="flex flex-wrap sm:container justify-end sm:mx-auto">
			<p class="text-xs md:max-w-sm p-2">
				We use cookies and similar technologies to enhance your experience on our site. This
				includes personalizing content, analyzing traffic, and improving our services. You have the
				option to opt-out of non-essential tracking. Learn more about our
			...
			<div class="xs:flex w-full md:w-40 md:block p-2 space-y-4">
				<button
					on:click={() => handleCookieConsent(true)}
				...
				<button
					on:click={() => handleCookieConsent(false)}
