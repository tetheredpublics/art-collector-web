...
<div
	...
	class="bg-appPeach fixed bottom-0 space-x-4 left-0 right-0 z-50 border-t border-black border-opacity-[0.16] px-4 py-4 flex items-center justify-between transition-transform duration-300 transform"
>
	{#if device === 'android'}
		<h2 class="md:text-lg font-bold text-black">Get Early Access</h2>
	{:else}
		<div class="flex items-center space-x-2">
			<img class="w-[24px] h-[24-px]" src="/icons/collect.png" alt="icon collect" />
			<h2 class="md:text-lg font-bold text-black">
				Start Your Collection <span class="hidden md:inline">Today</span>
			</h2>
		</div>
	{/if}
